...
def format_time(time: Union[str, int, float, datetime.datetime, datetime.date] = None,
                fmt='%Y-%m-%d %H:%M:%S') -> str:
    if time is None:
    ...
    elif not isinstance(time, (datetime.datetime, datetime.date)):
        # `datetime.datetime` is a subclass of `datetime.date`, both are accepted
